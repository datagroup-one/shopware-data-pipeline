...
class CRMProducer:
    ...
    def send_to_dlq(self, records: List[Dict], reason: str = "validation_failed") -> bool:
        ...
        try:
            ...
            dlq_key = f"dlq/crm/{reason}/{current_date}/dlq_{timestamp}.json"
            ...
            logger.info(f"Sent {len(records)} CRM records to DLQ: s3://{self.s3_bucket}/{dlq_key}.gz")
            return True
    ...
    def flush_s3_batch(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Flushing CRM S3 batch: {len(self.s3_batch_buffer)} records to s3://{self.s3_bucket}/{s3_key}.gz")
            ...
            logger.info(f"Successfully flushed {len(self.s3_batch_buffer)} CRM records to S3 (compressed)")
    ...
    def send_to_both_destinations(self, records: List[Dict]) -> Tuple[bool, bool]:
        """Send CRM data to both S3 (batched) and Firehose (immediate)"""
            ...
            if data:
                ...
                if valid_records:
                    ...
                    if s3_success and firehose_success:
                        logger.info("✅ Successfully processed CRM data to both S3 batch and Firehose")
                    elif s3_success:
